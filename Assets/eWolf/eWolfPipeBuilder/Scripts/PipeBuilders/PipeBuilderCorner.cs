    ...
    public partial class PipeBuilderCorner : PipeBuilderBase
    {
        ...
        public void CreatePipe(List<PipeNode> pipes, PipeNode pipeBase, SectionStraightBase straightBase)
        {
            ...
            for (float i = 0; i < 100 + angleSteps; i += angleSteps)
            {
                ...
                {
                    (float xFirst, float yFirst) = GetAngleSet(angle, _pipeBase.PipeSettings.JoinRadius);

                    Vector3 temp = endPos + ((midAngleTrist * xFirst)) + ((Up * yFirst));
        ...
        private void CreateBasicRing(Vector3[,] rings, int index, float pipeSides, Vector3 midAngleTrist, Vector3 Up, Vector3 endPos, float angle)
        {
            for (int j = 0; j < pipeSides + 1; j++)
            {
                (float xFirst, float yFirst) = GetAngleSet(angle, _pipeBase.PipeSettings.Radius);
        ...
        {
            float sectionBlock = _direction.magnitude * 4;
            float uVSectionLenth = ((1 / _pipeBase.PipeSettings.FlangeRadius) * (1 / sectionBlock)) * _pipeBase.PipeSettings.UVModifier;

            float partUVGap = uVSectionLenth / _pipeBase.PipeSettings.CornersDetail.Steps;
