...
namespace Wave.Essence.ScenePerception
{
	...
	{
		[SerializeField]
		public GameObject trackingOrigin = null;  // TODO be private or remove SerializeField

		private bool logPersisted = true;
		private bool logCached = true;
		...
		public WVR_Result CacheSpatialAnchor(WVR_SpatialAnchorName cachedSpatialAnchorName, UInt64 anchorHandle)
		{
			...
			if (result != WVR_Result.WVR_Success)
			{
				Log.e(LOG_TAG, "WVR_CacheSpatialAnchor failed with result " + result.ToString());
				return result;
			}
			else
			{
				if (logCached) Log.v(LOG_TAG, "WVR_CacheSpatialAnchor successful.");
			}
		...
		public WVR_Result UncacheSpatialAnchor(WVR_SpatialAnchorName cachedSpatialAnchorName)
		{
			...
			if (result != WVR_Result.WVR_Success)
			{
				Log.e(LOG_TAG, "WVR_UncacheSpatialAnchor failed with result " + result.ToString());
				return result;
			}
			else
			{
				if (logCached) Log.v(LOG_TAG, "WVR_UncacheSpatialAnchor successful.");
			}
		...
		public WVR_Result GetCachedSpatialAnchorNames(out WVR_SpatialAnchorName[] cachedSpatialAnchorNames)
		{
			...
			lock (lockObject)
			{
				...
				if (result != WVR_Result.WVR_Success)
				{
					Log.e(LOG_TAG, "WVR_EnumerateCachedSpatialAnchorNames 1 failed with result " + result.ToString());
					return result;
				}
			...
			if (result != WVR_Result.WVR_Success)
			{
				Log.e(LOG_TAG, "WVR_EnumerateCachedSpatialAnchorNames 2 failed with result " + result.ToString());
				return result;
			}
			else
			{
				if (logCached)
				{
					...
					if (cachedSpatialAnchorNames != null && cachedSpatialAnchorNames.Length > 0)
						foreach (var name in cachedSpatialAnchorNames)
							Log.v(LOG_TAG, TSCSB.Append("  ").Append(name.FromSpatialAnchorName()));
				}
			}
		...
		public WVR_Result ClearCachedSpatialAnchors()
		{
			...
			if (result != WVR_Result.WVR_Success)
			{
				Log.e(LOG_TAG, "WVR_ClearCachedSpatialAnchors failed with result " + result.ToString());
				return result;
			}
			else
			{
				if (logCached) Log.v(LOG_TAG, "WVR_ClearCachedSpatialAnchors successful.");
			}
		...
		{
			if (logCached) Log.v(LOG_TAG, $"CreateSpatialAnchorFromCacheName(san={spatialAnchorName}, can={cachedAnchorName})");
			...
			if (result != WVR_Result.WVR_Success)
			{
				Log.e(LOG_TAG, "WVR_CreateSpatialAnchorFromCacheName failed with result " + result.ToString());
				return result;
			}
			else
			{
				if (logCached) Log.v(LOG_TAG, "WVR_CreateSpatialAnchorFromCacheName successful.");
			}
		...
		public WVR_Result PersistSpatialAnchor(WVR_SpatialAnchorName persistedSpatialAnchorName, UInt64 anchor)
		{
			...
			if (result != WVR_Result.WVR_Success)
			{
				Log.e(LOG_TAG, "WVR_PersistSpatialAnchor failed with result " + result.ToString());
				return result;
			}
			else
			{
				if (logPersisted) Log.v(LOG_TAG, "WVR_PersistSpatialAnchor successful.");
			}
		...
		public WVR_Result UnpersistSpatialAnchor(string name)
		{
			...
			if (result != WVR_Result.WVR_Success)
			{
				Log.e(LOG_TAG, "WVR_UnpersistSpatialAnchor failed with result " + result.ToString());
				return result;
			}
			else
			{
				if (logPersisted) Log.v(LOG_TAG, "WVR_UnpersistSpatialAnchor successful.");
			}
		...
		public WVR_Result GetPersistedSpatialAnchorNames(out WVR_SpatialAnchorName[] persistedSpatialAnchorNames)
		{
			...
			lock (lockObject)
			{
				...
				if (result != WVR_Result.WVR_Success)
				{
					Log.e(LOG_TAG, "WVR_EnumeratePersistedSpatialAnchorNames 1 failed with result " + result.ToString());
					return result;
				}
			...
			if (result != WVR_Result.WVR_Success)
			{
				Log.e(LOG_TAG, "WVR_EnumeratePersistedSpatialAnchorNames 2 failed with result " + result.ToString());
				return result;
			}
			else
			{
		...
		public WVR_Result ClearPersistedSpatialAnchors()
		{
			...
			if (result != WVR_Result.WVR_Success)
			{
				Log.e(LOG_TAG, "WVR_ClearPersistedSpatialAnchors failed with result " + result.ToString());
				return result;
			}
			else
			{
				if (logPersisted) Log.v(LOG_TAG, "WVR_ClearPersistedSpatialAnchors successful.");
			}
			...
			out WVR_PersistedSpatialAnchorCountGetInfo getInfo)
		{
			...
			if (result != WVR_Result.WVR_Success)
			{
				Log.e(LOG_TAG, "WVR_GetPersistedSpatialAnchorCount failed with result " + result.ToString());
				return result;
			}
			else
			{
				if (logPersisted) Log.v(LOG_TAG, "WVR_GetPersistedSpatialAnchorCount successful.");
			}
			...
			out UInt64 anchor /* WVR_SpatialAnchor* */)
		{
			...
			if (result != WVR_Result.WVR_Success)
			{
				Log.e(LOG_TAG, "WVR_CreateSpatialAnchorFromPersistenceName failed with result " + result.ToString());
				return result;
			}
			else
			{
				if (logPersisted) Log.v(LOG_TAG, "WVR_CreateSpatialAnchorFromPersistenceName successful.");
			}
			...
			out byte[] data)
		{
			...
			lock (lockObject)
			{
				...
				if (result != WVR_Result.WVR_Success)
				{
					Log.e(LOG_TAG, "WVR_ExportPersistedSpatialAnchor 1 failed with result " + result.ToString());
					return result;
				}
				...
				if (result != WVR_Result.WVR_Success)
				{
					Log.e(LOG_TAG, "WVR_ExportPersistedSpatialAnchor 2 failed with result " + result.ToString());
					return result;
				}
				else
				{
		...
		public WVR_Result ImportPersistedSpatialAnchor(byte[] data)
		{
			...
			Log.d(LOG_TAG, TSCSB.Append("ImportPersistedSpatialAnchor(").Append(data.Length).Append(")"));
			WVR_Result result;
			lock (lockObject)
			{
				...
				if (result != WVR_Result.WVR_Success)
				{
					Log.e(LOG_TAG, "WVR_ImportPersistedSpatialAnchor failed with result " + result.ToString());
					return result;
				}
				else
				{
					if (logPersisted) Log.v(LOG_TAG, "WVR_ImportPersistedSpatialAnchor successful.");
				}
			}
			return result;
		}
	...
	public static class AnchorExtensions
	{
		...
		{
			for (int i = 0; i < wvrName.name.Length; i++)
			{
				if (wvrName.name[i] == '\0')
					return new string(wvrName.name, 0, i);
			}
			return new string(wvrName.name);
		}
		...
		public static WVR_SpatialAnchorName ToSpatialAnchorName(this string name)
		{
			...
			{
				char[] anchorName = name.ToCharArray();
				// Because the anchorName has no zero ending, if use copyto, we need add a ending zero byte.
				//anchorName.CopyTo(anchorNameWVR.name, 0);
				// And avoid oversized array copy
				int M = Mathf.Clamp(anchorName.Length, 0, 255);
				int i;
				for (i = 0; i < M; i++)
					anchorNameWVR.name[i] = anchorName[i];
				anchorNameWVR.name[i + 1] = '\0';
			}
			else
			{
				anchorNameWVR.name[0] = '\0';
			}
			return anchorNameWVR;
		}
	}
}
