...
namespace Wave.Essence.ScenePerception.Sample
{
    public class GeneratedPlaneContainer : IDisposable
    {
        ...
        {
            foreach (var generatedPlane in generatedPlanes)
            {
                if (generatedPlane.go != null){
                    Renderer renderer = generatedPlane.go.GetComponent<Renderer>();
                    if (renderer != null)
                    {
        ...
        public GameObject GenerateNewGameObject(WVR_ScenePlane plane)
        {
